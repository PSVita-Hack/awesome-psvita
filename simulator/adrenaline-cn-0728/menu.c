...
static char *graphics_options[] = { "原始", "Bilinear", "Sharp bilinear", "Advanced AA", "LCD3x" };
static char *screen_mode_options[] = { "原始", "標準", "縮放", "全屏" };
static char *no_yes_options[] = { "關", "開" };
static char *yes_no_options[] = { "開", "關" };
static char *screen_size_options[] = { "2.0倍", "1.75倍", "1.5倍", "1.25倍", "1.0倍" };
static char *ms_location_options[] = { "ux0:pspemu", "uma0:pspemu", "imc0:pspemu" };
...
static MenuEntry main_entries[] = {
	...
	{ "退出模擬器程序", MENU_ENTRY_TYPE_CALLBACK, 0, ExitPspEmuApplication, NULL, NULL, 0 },
...
static MenuEntry settings_entries[] = {
	...
	{ "圖像大小 (PSP)", MENU_ENTRY_TYPE_OPTION, 0, NULL, &config.screen_size, screen_size_options, sizeof(screen_size_options) / sizeof(char **) },
	{ "圖像模式 (PS1)", MENU_ENTRY_TYPE_OPTION, 0, NULL, &config.screen_mode, screen_mode_options, sizeof(screen_mode_options) / sizeof(char **) },
	{ "記憶棒位置", MENU_ENTRY_TYPE_OPTION, 0, NULL, &config.ms_location, ms_location_options, sizeof(ms_location_options) / sizeof(char **) },
	{ "DS3/DS4控制器", MENU_ENTRY_TYPE_OPTION, 0, NULL, &config.use_ds3_ds4, no_yes_options, sizeof(no_yes_options) / sizeof(char **) },
	{ "跳過Adrenaline啓動畫面", MENU_ENTRY_TYPE_OPTION, 0, NULL, &config.skip_logo, no_yes_options, sizeof(no_yes_options) / sizeof(char **) },
};
...
static MenuEntry about_entries[] = {
	...
	{ "molecule小組(HENkaku)", MENU_ENTRY_TYPE_TEXT, WHITE, NULL, NULL, NULL, 0 },
	{ "frangarcj(圖像濾鏡)", MENU_ENTRY_TYPE_TEXT, WHITE, NULL, NULL, NULL, 0 },
	{ "xerpi(vita2dlib庫)", MENU_ENTRY_TYPE_TEXT, WHITE, NULL, NULL, NULL, 0 },
};
...
void drawMenu() {
	...
	if (menu_entries) {
		...
		if (tab_sel == 2 && menu_sel == 0 && config.graphics_filtering == 0) {
			char *title = "濾鏡選擇原始時，所有圖像相關的選項均無效。";
